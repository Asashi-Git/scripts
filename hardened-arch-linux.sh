...
confirm_action() {
    read -p "$(echo -e ${YELLOW}$1 ${NC}(y/n): )" choice
    case "$choice" in
        y|Y ) return 0 ;;
        n|N ) return 1 ;;
        * ) print_error "Invalid choice"; confirm_action "$1" ;;
    esac
}
...
configure_http_partition() {
    ...
    if confirm_action "Do you want to create an encrypted HTTP partition?"; then
        ...
        lvcreate -L 5G vg0 -n httpdata || { print_error "Failed to create logical volume"; return 1; }
        
        # Encrypt it
        cryptsetup luksFormat /dev/vg0/httpdata || { print_error "Failed to encrypt volume"; return 1; }
        
        # Open encrypted volume
        cryptsetup open /dev/vg0/httpdata crypthttp || { print_error "Failed to open encrypted volume"; return 1; }
        
        # Format as ext4
        mkfs.ext4 /dev/mapper/crypthttp || { print_error "Failed to format volume"; return 1; }
        
        # Create mount point
        mkdir -p /mnt/data/http || { print_error "Failed to create mount point"; return 1; }
        
        # Mount
        mount /dev/mapper/crypthttp /mnt/data/http || { print_error "Failed to mount volume"; return 1; }
    ...
    if confirm_action "Do you want to add more UFW rules?"; then
        while true; do
            ...
            case "$proto" in
                ...
                *) print_error "Invalid protocol. Using tcp/udp."; ufw allow "$port" ;;
            esac
